...
function. Have `recognize` locate its model files by absolute path (or
relative to the executable, not the working directory): the sink invokes
it from its own cwd, and a relative `../models/...` path makes every
frame report zero faces with the error hidden in swallowed stderr.
...
If ffmpeg reports "Failed to resolve hostname" while curl to the same
host works, your ffmpeg build's resolver is broken (common in static
builds): use the curl-loop fallback in the `momento-streaming-origin`
skill.
